...
class FacePipeline:
    ...
    def __init__(self) -> None:
        ...
        # asset_id -> (asset_path, remaining_prerequisites)
    ...
    def register(self, asset_id: int, asset_path: str, prereq_count: int) -> None:
        ...
        """
        if prereq_count <= 0:
            self._schedule_face(asset_id, asset_path)
            return
        with self._lock:
            self._pending[asset_id] = (asset_path, prereq_count)
    ...
    def signal(self, asset_id: int) -> None:
        ...
        no-op.
        """
        enqueue_path: str | None = None
        with self._lock:
            entry = self._pending.get(asset_id)
            if entry is None:
                return
            asset_path, remaining = entry
            remaining -= 1
            if remaining <= 0:
                del self._pending[asset_id]
                enqueue_path = asset_path
            else:
                self._pending[asset_id] = (asset_path, remaining)

        if enqueue_path is not None:
            self._schedule_face(asset_id, enqueue_path)

    def _schedule_face(self, asset_id: int, asset_path: str) -> None:
        ...
        asyncio.run_coroutine_threadsafe(enqueue_face(asset_id, asset_path), self._loop)
