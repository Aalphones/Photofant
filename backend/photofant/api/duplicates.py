"""Duplicates API — CLIP duplicate search within a person's assets (ADR-007, ADR-018).
...
async def search_person_duplicates(
    ...
    """Find duplicate pairs among a person's assets via CLIP embeddings (ADR-018)."""
    ...
    rows = session.execute(
        select(AssetInstance.asset_id, Asset.content_hash, Asset.clip_embedding)
    ...
    clip_assets = [(int(row[0]), bytes(row[2])) for row in rows if row[2] is not None]
