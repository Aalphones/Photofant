...
class FluxEditRequest(BaseModel):
    prompt: str | None = None
    template_id: int | None = None
    params: dict[str, Any] = {}  # type: ignore[type-arg]


class FluxEditStarted(BaseModel):
    job_id: str


@router.post("/{asset_id}/flux-edit", response_model=FluxEditStarted, status_code=202)
async def flux_edit_asset(asset_id: int, body: FluxEditRequest, session: DbSession) -> FluxEditStarted:
    """Queue a GPU Flux-Edit (img2img) job. Result stored as a new version (type=flux_edit)."""
    from photofant.inference.generative_engine import GenerativeAvailability, check_generative_available
    ...
    if availability is not GenerativeAvailability.AVAILABLE:
        raise HTTPException(
            status_code=503,
            detail="Generative Abhängigkeiten nicht installiert. Zuerst installieren unter Einstellungen → Modelle.",
        )

    # Resolve prompt: from template_id or direct prompt
    prompt = (body.prompt or "").strip()
    if body.template_id is not None:
        from photofant.db.models import PromptTemplate
        template = session.get(PromptTemplate, body.template_id)
        if template is None:
            raise HTTPException(status_code=404, detail="Template not found")
        prompt = template.prompt
        # Merge template params as defaults, request params override
        merged_params = {**(template.params or {}), **body.params}
    else:
    ...
    # Replace {person} placeholder if asset has a person assignment
    asset, instance = row
    if "{person}" in prompt:
        person = session.get(Person, instance.person_id)
        person_name = person.name if person and not person.is_unknown else "person"
        prompt = prompt.replace("{person}", person_name)

    from photofant.jobs.flux_edit_job import enqueue_flux_edit
    status = await enqueue_flux_edit(asset_id, prompt, merged_params)
    return FluxEditStarted(job_id=status.id)


class BulkFluxEditRequest(BaseModel):
    asset_ids: list[int]
    prompt: str | None = None
    template_id: int | None = None
    params: dict[str, Any] = {}  # type: ignore[type-arg]
...
@router.post("/bulk-flux-edit", response_model=JobStarted, status_code=202)
async def bulk_flux_edit_assets(body: BulkFluxEditRequest, session: DbSession) -> JobStarted:
    """Queue Flux-Edit jobs for multiple assets (serial). Inpaint is single-only."""
    from photofant.inference.generative_engine import GenerativeAvailability, check_generative_available
    ...
    if availability is not GenerativeAvailability.AVAILABLE:
        raise HTTPException(
            status_code=503,
            detail="Generative Abhängigkeiten nicht installiert.",
        )
    ...
    # Enqueue per asset — {person} replaced per-asset
    first_status = None
    for aid in body.asset_ids:
        asset_prompt = prompt
        if "{person}" in asset_prompt:
            row = _active_row(session, aid)
            if row is not None:
                _, inst = row
                person = session.get(Person, inst.person_id)
                person_name = person.name if person and not person.is_unknown else "person"
                asset_prompt = asset_prompt.replace("{person}", person_name)
        job_status = await enqueue_flux_edit(aid, asset_prompt, merged_params)
        if first_status is None:
            first_status = job_status

    if first_status is None:
        raise HTTPException(status_code=422, detail="No assets to process")
    return JobStarted(job_id=first_status.id)
...
@router.post("/{asset_id}/inpaint", response_model=InpaintStarted, status_code=202)
async def inpaint_asset(asset_id: int, body: InpaintRequest, session: DbSession) -> InpaintStarted:
    """Queue a GPU Inpaint job. Result stored as a new version (type=inpaint). Single-asset only."""
    from photofant.inference.generative_engine import GenerativeAvailability, check_generative_available
    ...
    if not body.mask:
        raise HTTPException(status_code=422, detail="mask (base64 PNG) is required")
    ...
    from photofant.jobs.inpaint_job import enqueue_inpaint
    status = await enqueue_inpaint(asset_id, body.prompt, body.mask, body.params)
    return InpaintStarted(job_id=status.id)
