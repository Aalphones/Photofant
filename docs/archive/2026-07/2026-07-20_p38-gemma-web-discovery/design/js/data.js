...
(function () {
  ...
  // ---- Wissen (knowledge base) — mock entities per Hauptperson (P22/P23/P27 konzeptuell) ----
  const KFIELD_DEFS = [
    { key: "geburtstag", label: "Geburtstag" },
    { key: "beruf", label: "Beruf" },
    { key: "wohnort", label: "Wohnort" },
    { key: "vorlieben", label: "Vorlieben" },
    { key: "beziehung", label: "Beziehungsstatus" },
  ];
  const f = (value, owner, confidence) => ({ value, owner, confidence });
  const KNOWLEDGE = {
    0: { // Lena
      domain: "Familie", completeness: 0.82, updated: new Date(2026, 5, 18),
      fields: {
        geburtstag: f("14. März 1994", "user", 1),
        beruf: f("UX-Designerin", "web", 0.74),
        wohnort: f("Hamburg", "user", 1),
        vorlieben: f("Fotografie, Wandern, Katzen", "user", 1),
        beziehung: f(null, null, 0),
      },
      body: "Lena ist die ruhige Beobachterin der Familie — meistens mit Kamera in der Hand. Sie liebt lange Wanderungen an der Küste und verbringt Wochenenden am liebsten mit ihrer Katze Mimi und einem guten Buch.",
      relationships: [{ type: "Partner:in", targetPersonId: 1 }, { type: "Schwester", targetPersonId: 2 }],
      sources: [{ label: "Interview · 12. Feb 2026", kind: "interview" }, { label: "instagram.com/lena.mueller", kind: "web" }],
    },
    1: { // Jonas
      domain: "Familie", completeness: 0.45, updated: new Date(2026, 2, 3),
      fields: {
        geburtstag: f(null, null, 0),
        beruf: f("Elektroingenieur", "user", 1),
        wohnort: f(null, null, 0),
        vorlieben: f("Klettern, Synthesizer, Kaffee", "user", 1),
        beziehung: f("Mit Lena zusammen", "user", 1),
      },
      body: "Jonas bastelt am liebsten an irgendetwas — Synthesizern, Klettergriffen oder der perfekten Kaffeeröstung. Kurze, prägnante Antworten, viel lieber zeigt er Dinge als sie zu erklären.",
      relationships: [{ type: "Partner:in", targetPersonId: 0 }],
      sources: [{ label: "Manuell ergänzt · 3. Jan 2026", kind: "manual" }],
    },
    2: { // Mara
      domain: "Familie", completeness: 0.6, updated: new Date(2025, 10, 28),
      fields: {
        geburtstag: f("2. September 1991", "user", 1),
        beruf: f(null, null, 0),
        wohnort: f("Berlin", "web", 0.68),
        vorlieben: f("Kochen, Reisen, Vintage-Mode", "user", 1),
        beziehung: f(null, null, 0),
      },
      body: "Mara ist die Reiselustige — kaum ein Monat vergeht ohne einen neuen Ort auf ihrem Feed. Zuhause kocht sie aufwendige Menüs für die ganze Familie und sammelt Vintage-Kleider vom Flohmarkt.",
      relationships: [{ type: "Schwester", targetPersonId: 0 }],
      sources: [{ label: "Interview · 28. Nov 2025", kind: "interview" }],
    },
    3: { // Felix
      domain: "Familie", completeness: 0.9, updated: new Date(2026, 4, 5),
      fields: {
        geburtstag: f("30. Juni 1988", "user", 1),
        beruf: f("Physiotherapeut", "user", 1),
        wohnort: f("München", "user", 1),
        vorlieben: f("Triathlon, Grillen, Skandinavien-Krimis", "user", 1),
        beziehung: f("Verheiratet", "user", 1),
      },
      body: "Felix trainiert für seinen dritten Triathlon und liest nebenbei einen Krimi nach dem anderen. Er ist der verlässliche Fels in der Familie — immer der Erste, der beim Umzug anpackt.",
      relationships: [{ type: "Bester Freund", targetPersonId: 5 }],
      sources: [{ label: "Interview · 5. Mai 2026", kind: "interview" }, { label: "linkedin.com/in/felix-b", kind: "web" }],
    },
    4: { // Sofia
      domain: "Familie", completeness: 0.2, updated: new Date(2026, 1, 14),
      fields: {
        geburtstag: f(null, null, 0),
        beruf: f(null, null, 0),
        wohnort: f(null, null, 0),
        vorlieben: f("Zeichnen, Anime", "user", 1),
        beziehung: f(null, null, 0),
      },
      body: "Erst wenige Notizen vorhanden — bislang nur bekannt, dass Sofia gerne zeichnet.",
      relationships: [],
      sources: [{ label: "Manuell ergänzt · 14. Feb 2026", kind: "manual" }],
    },
    5: null, // Noah — noch keine Entity angelegt
  };
  const KNOWLEDGE_TASKS = [
    { id: 1, personId: 1, kind: "missing_field", label: "Geburtstag & Wohnort fehlen", sub: "Jonas" },
    { id: 2, personId: 2, kind: "missing_field", label: "Beruf fehlt", sub: "Mara" },
    { id: 3, personId: 4, kind: "low_completeness", label: "Profil kaum ausgefüllt (20%)", sub: "Sofia" },
    { id: 4, personId: 5, kind: "no_entity", label: "Noch kein Wissen angelegt", sub: "Noah · 12 Fotos ohne Profil" },
    { id: 5, personId: 3, kind: "suggestion", label: "KI-Vorschlag verfügbar", sub: "Felix · Beruf laut LinkedIn bestätigen" },
    { id: 6, personId: 5, entityId: "u1", kind: "auto_link", label: "Notiz „Noah B.“ ähnelt Person Noah", sub: "96% Namens-Übereinstimmung — verknüpfen?" },
  ];
  // Wissens-Notizen ohne (bekannte) Foto-Person — z.B. per Web-Suche/Interview mit freiem
  // Namen angelegt oder noch nicht bestätigt zugeordnet. Manuelle Verknüpfung im UI (Wissen-Tab).
  const KNOWLEDGE_UNLINKED = [
    { id: "u1", title: "Noah B.", domain: "Familie", completeness: 0.35, updated: new Date(2026, 6, 2),
      fields: { geburtstag: f(null, null, 0), beruf: f(null, null, 0), wohnort: f("Freiburg", "web", 0.7), vorlieben: f("Gitarre, Bouldern", "user", 1), beziehung: f(null, null, 0) },
      body: "Per Interview angelegt, aber noch keiner Person aus der Fotobibliothek zugeordnet.",
      relationships: [], matchPersonId: 5, matchConfidence: 0.96,
      sources: [{ label: "Interview · 2. Jul 2026", kind: "interview" }] },
    { id: "u2", title: "Opa Werner", domain: "Familie", completeness: 0.5, updated: new Date(2026, 5, 20),
      fields: { geburtstag: f("11. April 1952", "user", 1), beruf: f("Rentner, ehem. Schreiner", "user", 1), wohnort: f("Freiburg", "user", 1), vorlieben: f(null, null, 0), beziehung: f(null, null, 0) },
      body: "Familienwissen ohne Fotos in der aktuellen Bibliothek — keine Person zum Verknüpfen vorhanden.",
      relationships: [], matchPersonId: null, matchConfidence: 0,
      sources: [{ label: "Manuell ergänzt · 20. Jun 2026", kind: "manual" }] },
  ];
  // KI-Vorschläge für fehlende Felder, angezeigt im Wissens-Detail (P27-Konzept)
  const KNOWLEDGE_SUGGESTIONS = {
    1: { text: "Aus 6 Fotos mit dem Tag „outdoor“ + Bildunterschriften lässt sich ein Wohnort-Hinweis ableiten.", field: "wohnort", value: "vermutlich Freiburg im Breisgau" },
    2: { text: "3 Fotos mit Bürokleidung deuten auf einen Beruf im Consulting hin.", field: "beruf", value: "vermutlich Unternehmensberaterin" },
  };
  // Web-Suche-Wizard: kanonische, per Domain konfigurierbare Vorschlagsfelder
  const KNOWLEDGE_WEB_FACTS = [
    { field: "Beruf", value: "UX-Designerin bei einer Hamburger Agentur", source: "linkedin.com", confidence: 0.81 },
    { field: "Wohnort", value: "Hamburg, Deutschland", source: "instagram.com", confidence: 0.74 },
    { field: "Ausbildung", value: "B.A. Kommunikationsdesign, HAW Hamburg", source: "xing.com", confidence: 0.66 },
  ];
  const KNOWLEDGE_INTERVIEW_QUESTIONS = [
  ...
  window.PF = {
    ...
    KFIELD_DEFS, KNOWLEDGE, KNOWLEDGE_TASKS, KNOWLEDGE_UNLINKED, KNOWLEDGE_SUGGESTIONS, KNOWLEDGE_WEB_FACTS, KNOWLEDGE_INTERVIEW_QUESTIONS,
