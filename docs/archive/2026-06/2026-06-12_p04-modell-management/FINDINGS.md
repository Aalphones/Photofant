...
- [ ] → Core-Inferenz-Phase (P5+): `onnxruntime` ist noch keine Backend-Dependency. Die In-Place-Validierung (Phase 3) macht Probe-Load (Stufe 5) per Protobuf-Header-Sniff und Rollen-Check (Stufe 3) nur opportunistisch, wenn die Runtime da ist. Sobald `onnxruntime` als Pflicht-Dependency eingezogen wird (python.md nennt es so), greift die echte ONNX-Session automatisch — Code ist schon vorbereitet (`_open_onnx_session`). Dann ggf. Rollen-Erwartungs-Tabelle (Input-Shape pro Rolle) verfeinern.
