...
- [ ] Workspace generieren (`ng new`), Prefix `pf`, SCSS, strict; tsconfig-Flags + Aliases `@photofant/*` mit Barrels
- [ ] Tailwind v4 + `@theme`-Block aus `docs/design/styles.css` übertragen; IBM Plex Mono einbinden (lokal, kein CDN — Offline-Prinzip)
- [ ] App-Shell-Komponenten: `shell`, `nav-rail`, `top-bar` (BEM + scoped SCSS, `:host`-Regeln beachten)
- [ ] Routen-Gerüst mit `loadComponent`-Platzhaltern
- [ ] NgRx-Grundverdrahtung (`provideStore`, `provideEffects`), `store/jobs/`-Slice nach ngrx.md inkl. SSE-Effect (EventSource → Actions)
- [ ] Job-Pill (Spinner-Zustand) + Job-Dock (Desktop-Popover, Mobile-Sheet) gegen den Demo-Job
- [ ] Dev-Proxy `/api` → Backend-Port
- [ ] Doc-Update: AGENTS.md Stack-Tabelle (Angular-Major, Test-Runner pinnen)
