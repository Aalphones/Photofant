...
- [ ] → Phase 2: Die API-Karte heißt `{ asset_id, thumbnail_url, score, reasons[] }` — die generische
...
- [ ] → Phase 2: `GET /api/recommendations?asset_id=` liefert `status: "ready"|"computing"|"disabled"`. Bei
...
- [ ] → Phase 2: Eine Empfehlungs-Karte kann **mehrere** Signal-Häkchen tragen (same_person + same_role +
