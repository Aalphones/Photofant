...
(function () {
  ...
  function Nav({ view, setView, assets, onClose }) {
    ...
    const items = [
      ...
      { id: "wissen", icon: "book", label: "Wissen", count: PF.KNOWLEDGE_TASKS.length },
  ...
  function JobDock({ jobs, onClose }) {
    const iconFor = (k) => ({ tag: "tag", face: "face", caption: "text", download: "download", comfy: "sparkle" }[k] || "refresh");
  ...
  /* ---------------- placeholder views ---------------- */
  function PeopleView({ assets, onPickPerson, onImport, knowledgeFor, onOpenKnowledge }) {
    ...
    return React.createElement("div", { className: "grid-wrap" },
      ...
      React.createElement("div", { className: "people-grid" },
        PF.PERSONS.map((p) => React.createElement("div", {
          ...
          (() => {
            const ent = knowledgeFor(p.id);
            return ent
              ? React.createElement("button", {
                  className: "person-know-chip", title: "Wissen · " + Math.round(ent.completeness * 100) + "%",
                  onClick: (e) => { e.stopPropagation(); onOpenKnowledge(p.id); },
                }, React.createElement(Icon, { name: "sparkle", size: 11 }), Math.round(ent.completeness * 100) + "%")
              : React.createElement("button", {
                  className: "person-know-nudge", title: "Wissen anlegen",
                  onClick: (e) => { e.stopPropagation(); onOpenKnowledge(p.id); },
                }, React.createElement(Icon, { name: "book", size: 11 }), "Wissen anlegen?");
          })(),
  ...
  function App() {
    ...
    const [wissenFocus, setWissenFocus] = useState(null);
    const [kLinks, setKLinks] = useState({});
    const [kUnlinked, setKUnlinked] = useState(PF.KNOWLEDGE_UNLINKED);
    const knowledgeFor = (pid) => Object.prototype.hasOwnProperty.call(kLinks, pid) ? kLinks[pid] : PF.KNOWLEDGE[pid];
    const openKnowledge = (pid) => { setLb(null); setWissenFocus(pid); goView("wissen"); };
    const jumpLb = (id) => setLb((cur) => ({ id, order: cur && cur.order.includes(id) ? cur.order : assets.map((a) => a.id) }));
    // ComfyUI run-mode: gallery is the picker. A click on a cell binds to the armed slot.
    const comfy = window.ComfyUI.useRun({ assets, pushJobs, updateAsset });
    const handleGalleryOpen = (id, order) => { if (comfy.tryBind(id)) return; openLb(id, order); };
    ...
    return React.createElement("div", {
      ...
      React.createElement("div", { className: "main" + (comfy.active ? " cf-run" : "") },
        // top bar
        React.createElement("div", { className: "top" },
          ...
          React.createElement("div", { className: "top-actions" },
            ...
            isGalleryLike && React.createElement("button", { className: "iconbtn" + (comfy.active ? " on" : ""), title: "ComfyUI-Workflow", onClick: () => comfy.active ? comfy.exit() : comfy.enter() },
        ...
        isGalleryLike
          ? React.createElement(Gallery, {
              ...
              search, onOpen: handleGalleryOpen, onFav: toggleFav,
              sel, setSel, selMode, setSelMode, sort, setSort, group, setGroup, density, setDensity,
              railOpen, setRailOpen, isMobile, comfy,
            })
          : view === "people"
            ? React.createElement(PeopleView, { assets, onImport: openImport, onPickPerson: (pid) => { goView("gallery"); setF({ ...emptyFilters(), persons: new Set([pid]) }); }, knowledgeFor, onOpenKnowledge: openKnowledge })
            : view === "wissen"
              ? React.createElement(window.Knowledge, { assets, focusPersonId: wissenFocus, onOpenAsset: (id) => { setLb({ id, order: assets.map((a) => a.id) }); }, onConsumeFocus: () => setWissenFocus(null), linkOverrides: kLinks, setLinkOverrides: setKLinks, unlinked: kUnlinked, setUnlinked: setKUnlinked })
            : view === "albums"
      ...
      // ComfyUI docked run-bar (only in gallery run mode)
      comfy.active && window.ComfyUI.RunBar && React.createElement(window.ComfyUI.RunBar, { comfy })),
      ...
      lbAsset && React.createElement(Lightbox, {
        ...
        onImport: openImport, onEdit: openEditor, onOpenKnowledge: openKnowledge, onJumpTo: jumpLb,
      }),
      ...
      // ComfyUI mask editor + output import (fixed overlays)
      comfy.active && window.ComfyUI.MaskModal && React.createElement(window.ComfyUI.MaskModal, { comfy }),
      comfy.active && window.ComfyUI.OutputDialog && React.createElement(window.ComfyUI.OutputDialog, { comfy }));
  }
