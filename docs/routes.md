...
| Interview-Mode (P27 Phase 4) | `POST` | `/api/knowledge/ai/interview` | `InterviewSynthesizeRequest { title, domain, type, answers: { question, answer }[], person_ids?, asset_ids? }` | `{ job_id }` — startet `InterviewJob` **asynchron**; Ergebnis (`KnowledgeInterviewResult`) über den Job-Stream. 409 wenn `ai.autonomy.interview == "off"`, 422 bei leerem Namen **oder** nicht-privater Domäne (Interview ist nur für private Domänen — Konzept-ADR-009) |
...
**Interview-Ergebnis (P27 Phase 4, über `JobDto.result` des `interview`-Jobs):** `KnowledgeInterviewResult { suggestion: { title, type, domain, aliases, relationships, body } | null, explainability: {…}, validation_errors: string[] }` — gleiche Form wie der Import-Vorschlag. Gemma **synthetisiert** den Body ausschließlich aus den Interview-Antworten (kein Web, kein Bestand — Konzept-ADR-009). `suggestion` ist `null` bei Validator-Abweisung; angelegt wird erst durch die Nutzer-Bestätigung über `POST /knowledge/entities` (`owner=user`). `GET /api/knowledge/domains` liefert je Domäne zusätzlich `private: boolean` (privat markierte Domänen).
