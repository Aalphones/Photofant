...
| `/galerie` (Bulk-Bar „Zu Album") | `POST` | `/api/collections/{id}/items` | `{ asset_ids?: number[], face_ids?: number[] }` (Gesichter-Mehrfachauswahl) | `204` — als `source=manual` |
| `/alben` (Mitglied entfernen) | `DELETE` | `/api/collections/{id}/items/{asset_id}` | — | `204` |
| `/trainingssets` (Face-Item entfernen · Gesichter-Mehrfachauswahl) | `DELETE` | `/api/collections/{id}/items/faces/{face_id}` | — | `204` — Geschwister-Route zur Asset-Route, **muss** vor `/items/{asset_id}` registriert sein (sonst verdeckt Starlette sie über Pfad-Segment-Matching) |
...
| `/trainingssets` (Item-Grid · P10 Phase 2, Face-Items seit Gesichter-Mehrfachauswahl) | `GET` | `/api/collections/{id}/items` | — | `TrainingSetItemDto[]` — eigenes Read-Model (Caption/Tags/Qualität), dünneres `AssetDto` der Galerie bleibt unangetastet; Face-Items (`kind="face"`) liefern `face_id`/`thumbnail_url` statt `content_hash`, `tags: []`, keine Framing/Quality |
...
| `/trainingssets` (Caption-Override Face-Item · Gesichter-Mehrfachauswahl) | `PATCH` | `/api/collections/{id}/items/faces/{face_id}` | `{ caption_override: string \| null }` | `204` — Faces haben keine Original-Caption, das ist die einzige Caption-Quelle; **muss** vor `/items/{asset_id}` registriert sein |
...
| `/galerie` (Bulk-Bar „Zu Trainingsset" · P10 Phase 2, Face-Items seit Gesichter-Mehrfachauswahl) | `POST` | `/api/collections/{id}/items` | `{ asset_ids?: number[], face_ids?: number[] }` | `204` — gleicher Endpoint wie „Zu Album", nur andere Ziel-Collection (`kind=training_set`) |
...
| Einstellungen › KI (Teil-Update) | `PATCH` | `/api/knowledge/ai/autonomy` | `AutonomyPatchRequest { knowledge_import?, knowledge_update?, interview?, discovery? }` — nur angegebene Felder ändern sich | `AutonomyDto` (frischer Gesamtstand) — schreibt sofort nach `settings.json`, kein Neustart nötig. 422 bei unbekanntem Modus oder `discovery: "ask"` (kennt nur `off\|auto`, ADR-031) |
...
| Lore Panel „Ergänzen (KI)" (P27 Phase 3) | `POST` | `/api/knowledge/ai/update-suggestion` | `UpdateSuggestionRequest { entity_id }` | `{ job_id }` — startet `KnowledgeUpdateJob` **asynchron**; Ergebnis (`KnowledgeUpdateResult`) über den Job-Stream. 409 wenn `ai.autonomy.knowledge_update == "off"`, 422 bei leerer id |
| Ergänzung annehmen (P27 Phase 3) | `POST` | `/api/knowledge/ai/update-suggestion/accept` | `AcceptUpdateSuggestionRequest { entity_id, body, reason }` | `{ job_id }` — schreibt über den P25-Patch-Pfad mit `owner=inferred` (serverseitig fix, **nicht** `/patch` das `owner=user` setzt) |
| Interview-Mode (P27 Phase 4, erweitert P39 Phase 2) | `POST` | `/api/knowledge/ai/interview` | `InterviewSynthesizeRequest { title, domain, type, answers: { question, answer, field_key? }[], person_ids?, asset_ids? }` — `field_key` setzt eine Antwort auf ein Merkmal des Typs (Wizard-Schritt „Eckdaten"); solche Werte werden wörtlich übernommen, ein der Domäne unbekannter Key wird verworfen | `{ job_id }` — startet `InterviewJob` **asynchron**; Ergebnis (`KnowledgeInterviewResult`) über den Job-Stream. 409 wenn `ai.autonomy.interview == "off"`, 422 bei leerem Namen **oder** nicht-privater Domäne (Interview ist nur für private Domänen — Konzept-ADR-009) |

**Ergänzungs-Ergebnis (P27 Phase 3, über `JobDto.result` des `knowledge_update`-Jobs):** `KnowledgeUpdateResult { proposal: { body } | null, old_body: string, explainability: {…}, validation_errors: string[] }` — Diff-Basis fürs Lore Panel; `proposal` ist `null` bei Validator-Abweisung. Ist die Entity mit einer Person verknüpft, fließen zusätzlich Captions + häufigste Tags von deren erkannten Fotos als Hinweis (kein Beleg) in den Prompt ein — kein neues Feld, keine neue DTO (P39 Phase 8).

**Interview-Ergebnis (P27 Phase 4, erweitert P39 Phase 2, über `JobDto.result` des `interview`-Jobs):** `KnowledgeInterviewResult { suggestion: { title, type, domain, aliases, relationships, body, attributes: Record<key, { label, value, owner, confidence }> } | null, explainability: {…}, validation_errors: string[] }` — gleiche Form wie der Import-Vorschlag. Gemma **synthetisiert** den Body ausschließlich aus den Interview-Antworten (kein Web, kein Bestand — Konzept-ADR-009). Die Merkmals-Keys stammen ausschließlich aus `fields_for` der Domäne; ein selbst eingetippter Wert trägt `owner: "user"` und Confidence 1.0, ein Modell-Vorschlag für ein leer gelassenes Feld `owner: "inferred"` (ADR-034). Das `label` reist mit, damit der Wizard keine eigene Domänen-Auflösung braucht. `suggestion` ist `null` bei Validator-Abweisung; angelegt wird erst durch die Nutzer-Bestätigung über `POST /knowledge/entities` (`owner=user`). `GET /api/knowledge/domains` liefert je Domäne zusätzlich `private: boolean` (privat markierte Domänen).
...
**`LoreDto` (P25 Phase 1 — Vollform des P22-Stubs, erweitert P39 Phase 7):** `{ entity: EntityDto | null, relationships: { type, target: EntityRefDto }[], franchises: EntityRefDto[], related_media: { kind: "person"|"asset", id, thumbnail_url, label? }[], related_photos_total: number, sources: string[] }`. `EntityRefDto = { id, title, type, completeness }`. Beziehungsziele sind 1 Hop aufgelöst (Titel+Typ statt roher id) — Ziel nicht im Cache → Titel fällt auf die rohe id zurück, keine Beziehung geht verloren. `franchises` ist eine Teilmenge von `relationships` (Ziel-Typ `"Franchise"`), eigenes Feld weil das Lore-Panel es als eigene Sektion zeigt. `related_media` löst `media_links` (rohe Personen-/Asset-ids) zu Thumbnails auf — Personen ohne Gesichts-Aufnahme (kein Portrait) werden ausgelassen. Ist mindestens eine Person verknüpft, kommen zusätzlich deren **live** über `AssetInstance` gelesenen erkannten Fotos als `kind="asset"` dazu — dedupliziert gegen manuell verknüpfte Assets, nie in den Vault zurückgeschrieben. Gedeckelt auf die neuesten 24 (`_MAX_RELATED_PHOTOS`); `related_photos_total` ist die ungedeckelte Gesamtzahl, damit das Frontend „und N weitere" zeigen kann.
...
interface EntityDto {
  ...
  owner: "user" | "manual" | "web" | "inferred"; confidence: number; status: string;
  aliases: string[];
