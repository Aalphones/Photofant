...
**Ergänzungs-Ergebnis (P27 Phase 3, über `JobDto.result` des `knowledge_update`-Jobs):** `KnowledgeUpdateResult { proposal: { body } | null, old_body: string, explainability: {…}, validation_errors: string[] }` — Diff-Basis fürs Lore Panel; `proposal` ist `null` bei Validator-Abweisung.
