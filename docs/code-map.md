...
| **Klassifizierung & Heuristik** | `services/classify.service.ts` | `api/classify.py` (Rerun-Step `categories`) · `jobs/heuristics_job.py` · `classification/engine.py` (CLIP+WD14-Fusion, kein Modell-Neulauf), `classification/scoring.py`, `classification/seed.py` · `jobs/classification_job.py`, `jobs/classification_pipeline.py` (wartet auf Tagging+Embedding) |
