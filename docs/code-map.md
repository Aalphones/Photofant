...
| **Suche** (Freitext-fuzzy/Tag/Caption/semantisch) | `ui/search-box/` · `store/search/` · `services/search.service.ts` | `api/assets.py` (`list_assets`, alle `q_mode`-Zweige inkl. Fuzzy-Freitext via `rapidfuzz`, ADR-015) · `api/search.py` (`POST /api/search/warm` Prewarm gegen CLIP-Kaltstart, P28 Phase 3; toter `POST /api/search/semantic`-Endpoint, kein Frontend-Aufrufer) · `db/vector_index.py` · `jobs/embedding_job.py` · `inference/adapters/clip.py` |
