...
| **Suche** (Freitext-fuzzy/Tag/Caption/semantisch) | `ui/search-box/` · `store/search/` | `api/assets.py` (`list_assets`, alle `q_mode`-Zweige inkl. Fuzzy-Freitext via `rapidfuzz`, ADR-015) · `api/search.py` (toter `POST /api/search/semantic`-Endpoint, kein Frontend-Aufrufer) · `db/vector_index.py` · `jobs/embedding_job.py` · `inference/adapters/clip.py` |
