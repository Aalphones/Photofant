...
| **Suche** (Freitext exakt/Prefix, Tag, Caption, semantisch) | `ui/search-box/` · `store/search/` | `api/assets.py` (`list_assets`, alle `q_mode`-Zweige; `q_mode=text` = Tag-/Personen-Name ILIKE + Caption via FTS5, Fuzzy-Toleranz entfernt, ADR-015-Nachtrag) · `db/text_index.py` (FTS5-Caption-Index `asset_caption_fts`) · `api/search.py` (toter `POST /api/search/semantic`-Endpoint, kein Frontend-Aufrufer) · `db/vector_index.py` · `jobs/embedding_job.py` · `inference/image_embedder.py` (Capability-Resolver `resolve_image_embedder`, ADR-022) · `inference/adapters/clip.py` · `inference/adapters/siglip.py` |
...
| **Einstellungen** | `features/einstellungen/` (Shell + darstellung, bibliothek, verarbeitung, bearbeitung, info, tastaturkuerzel, tags, klassifizierung, comfyui, mcp, backup-wartung) · `services/settings.service.ts` · `models/config.model.ts` | `api/config.py` · `settings.py`, `config.py` |
