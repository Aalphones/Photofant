...
| **Trainingssets & Export** | `features/trainingssets/` (trainingssets, training-set-item, training-set-settings, training-set-stats) · `store/collections/` · `services/collection.service.ts` | `api/collections.py` (`/items`, `/stats`) · `collections/stats.py` (AR-Buckets, Near-Dupe-Quote) |
