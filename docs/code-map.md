...
| **Klassifizierung & Heuristik** | `services/classify.service.ts` | `api/classify.py` (Rerun-Step `categories`) · `api/classification.py` (CRUD Kategorien/Labels, explizites Cascade-Delete — `PRAGMA foreign_keys` projektweit aus) · `api/assets.py` (`classification`-Filter OR/AND, Facet `classifications`, `AssetDetailDto.classifications`, Label-Treffer im `q_mode=text`) · `jobs/heuristics_job.py` · `classification/engine.py` (CLIP+WD14-Fusion, kein Modell-Neulauf), `classification/scoring.py`, `classification/seed.py` · `jobs/classification_job.py`, `jobs/classification_pipeline.py` (wartet auf Tagging+Embedding) |
